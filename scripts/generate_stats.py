...
def card_frame(width: int, height: int, title: str, title_emoji: str = "") -> str:
    return f"""
  <defs>
    ...
    <linearGradient id="card-glow" x1="0" y1="0" x2="1" y2="1">
      <stop offset="0%" stop-color="{ACCENT_PINK}" stop-opacity="0.08"/>
      <stop offset="100%" stop-color="{ACCENT_BLUE}" stop-opacity="0.06"/>
    </linearGradient>
  </defs>
  <rect width="{width}" height="{height}" rx="16" fill="{BG_DARK}"/>
  <rect width="{width}" height="{height}" rx="16" fill="url(#card-glow)"/>
  <rect width="{width}" height="{height}" rx="16" fill="none" stroke="url(#border)" stroke-width="2"/>
  <text x="24" y="36" font-family="{FONT}" font-size="17" font-weight="700" fill="{TXT_LIGHT}">{title_emoji} {title}</text>
  <line x1="24" y1="48" x2="{width - 24}" y2="48" stroke="{ACCENT_PURPLE}" stroke-width="1" opacity="0.4"/>
"""
...
def render_stats_card(user: dict, repos: list[dict]) -> str:
    ...
    rows = [
        ...
        ("📝 Public Gists", public_gists, ACCENT_BLUE),
    ]
    ...
    for i, (label, value, color) in enumerate(rows):
        y = 80 + i * 32
        body.append(
            f'  <text x="32" y="{y}" font-family="{FONT}" font-size="14" fill="{TXT_MUTED}">{label}</text>'
            f'  <text x="{width - 32}" y="{y}" text-anchor="end" font-family="{FONT}" font-size="18" font-weight="700" fill="{color}">{value}</text>'
        )

    return f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {width} {height}" role="img" aria-label="GitHub stats for {GH_USER}">
{card_frame(width, height, f"GitHub Stats · @{GH_USER}", "📊")}
{chr(10).join(body)}
</svg>
"""
...
def render_top_langs(lang_bytes: dict[str, int], top_n: int = 6) -> str:
    ...
    for i, (lang, bytes_) in enumerate(items):
        ...
        bar_w = max(1, int(bar_max * pct / 100))
        body.append(
            f'  <text x="32" y="{y + 12}" font-family="{FONT}" font-size="13" font-weight="600" fill="{TXT_LIGHT}">{lang}</text>'
            f'  <rect x="{bar_x}" y="{y}" width="{bar_max}" height="14" rx="7" fill="#1a1925"/>'
            f'  <rect x="{bar_x}" y="{y}" width="{bar_w}" height="14" rx="7" fill="{color}"/>'
            f'  <text x="{width - 32}" y="{y + 12}" text-anchor="end" font-family="{FONT}" font-size="12" font-weight="700" fill="{TXT_MUTED}">{pct:.1f}%</text>'
        )

    if not items:
        body.append(
            f'  <text x="{width//2}" y="{height//2}" text-anchor="middle" font-family="{FONT}" font-size="14" fill="{TXT_DIM}">No language data</text>'
        )

    return f"""<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {width} {height}" role="img" aria-label="Top languages for {GH_USER}">
{card_frame(width, height, "Most Used Languages", "🎨")}
{chr(10).join(body)}
</svg>
"""
...
{card_frame(width, height, f"LeetCode · @{LC_USER}", "🧩")}
...
"""
    ...
    rows = [
        ("Easy", easy, totals.get("Easy", 1) or 1, "#00B8A3"),
        ("Medium", medium, totals.get("Medium", 1) or 1, ACCENT_YELLOW),
        ("Hard", hard, totals.get("Hard", 1) or 1, "#FF375F"),
    ]

    body = [
        f'  <text x="32" y="86" font-family="{FONT}" font-size="14" fill="{TXT_MUTED}">Solved</text>',
        f'  <text x="32" y="116" font-family="{FONT}" font-size="32" font-weight="800" fill="{TXT_LIGHT}">{solved_all}<tspan font-size="16" fill="{TXT_DIM}"> / {total_all}</tspan></text>',
        f'  <text x="{width - 32}" y="86" text-anchor="end" font-family="{FONT}" font-size="13" fill="{TXT_MUTED}">Ranking</text>',
        f'  <text x="{width - 32}" y="116" text-anchor="end" font-family="{FONT}" font-size="22" font-weight="700" fill="{ACCENT_CYAN}">#{ranking}</text>',
    ]
    ...
    for i, (label, val, total, color) in enumerate(rows):
        ...
        bar_w = max(1, int(bar_max * pct / 100))
        body.extend([
            f'  <text x="32" y="{y + 12}" font-family="{FONT}" font-size="13" font-weight="600" fill="{color}">{label}</text>',
            f'  <rect x="{bar_x}" y="{y}" width="{bar_max}" height="14" rx="7" fill="#1a1925"/>',
            f'  <rect x="{bar_x}" y="{y}" width="{bar_w}" height="14" rx="7" fill="{color}"/>',
            f'  <text x="{width - 32}" y="{y + 12}" text-anchor="end" font-family="{FONT}" font-size="12" font-weight="700" fill="{TXT_MUTED}">{val} / {total}</text>',
        ])
...
{card_frame(width, height, f"LeetCode · @{LC_USER}", "🧩")}
  <text x="{width//2}" y="{height//2 - 6}" text-anchor="middle" font-family="{FONT}" font-size="14" fill="{TXT_MUTED}">Stats will appear after the next scheduled run.</text>
  <text x="{width//2}" y="{height//2 + 18}" text-anchor="middle" font-family="{FONT}" font-size="12" fill="{TXT_DIM}">(LeetCode often blocks non-CI clients)</text>
</svg>
"""
